...
WINDOW_WIDTH = 700
WINDOW_HEIGHT = 650
FIELD_SIZE = 488  # Увеличено на 5 пикселей вправо и вниз (было 480)
GRID_SIZE = 20
...
DEATH_SPEED = 2  # Скорость смерти змейки (чем меньше число, тем быстрее исчезают сегменты)

# Цвета
BLACK = (0, 0, 0)
GREEN = (0, 255, 0)
GREEN_ALPHA = (0, 255, 0, 128)  # Полупрозрачный зеленый для умирающей змейки (альфа = 128)
RED = (255, 0, 0)
WHITE = (255, 255, 255)
YELLOW = (255, 255, 0)
...
class Snake:
    ...
    def render(self, surface):
        for i, (x, y) in enumerate(self.positions):
            ...
            if self.dead:
                # Создаем временную поверхность с альфа-каналом для полупрозрачности
...
def main():
    ...
    while True:
        ...
        snake.render(screen)  # Змейка рисуется всегда, даже при исчезновении
        if not game_over and not snake.dead:  # Яблоко рисуем только если змейка жива
        ...
        # Текст проигрыша появляется сразу после смерти и остается после исчезновения
